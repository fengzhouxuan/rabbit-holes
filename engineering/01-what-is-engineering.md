...
- 和 [分层与封装](../networking/03-layering.md) 是同一个母题：复杂系统必须靠边界降低互相影响。
- 和 [数据库文件怎么存](../database/01-storage-pages-buffer-wal.md) 是同一个母题：底座能力很弱，上层机制补出可靠抽象。
- 和 [JavaScript 运行时](../javascript/01-js-runtime.md) 是同一个母题：真正能做事的不是孤立语法，而是一整套宿主能力和调度系统。
