...
- [HTTP / 缓存 / Cookie / Session](../networking/09-http-cache-cookie-session.md)：Cookie / Session 在无状态 HTTP 上补出身份连续性；数据库在普通文件上补出可靠账本。
- [负载均衡 / CDN](../networking/08-load-balancing-cdn.md)：应用服务器可以横向扩展，但长期业务事实通常要落到共享数据库。
- 母题 [在更弱的底座上补出更强的抽象](../patterns.md)。
